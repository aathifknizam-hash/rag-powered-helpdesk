"""
Vector Sync Manager - Phase 12: Vector Consistency
...
class VectorSyncManager:
    ...
    def _derive_department(self, category: str) -> str:
        category_lower = (category or '').strip().lower()
        if category_lower in ('leave management', 'payroll', 'benefits', 'hr'):
            return 'HR'
        if category_lower == 'facilities':
            return 'Facilities'
        if category_lower == 'finance':
            return 'Finance'
        if category_lower in ('vpn', 'networking', 'email', 'hardware', 'software', 'windows', 'security', 'account', 'support'):
            return 'Technical Support'
        return 'General Support'

    def _derive_source(self, category: str) -> str:
        category_lower = (category or '').strip().lower()
        if category_lower in ('leave management', 'payroll', 'benefits', 'hr'):
            return 'HR Knowledge Base'
        if category_lower == 'facilities':
            return 'Facilities Knowledge Base'
        if category_lower == 'finance':
            return 'Finance Knowledge Base'
        if category_lower in ('vpn', 'networking', 'email', 'hardware', 'software', 'windows', 'security', 'account', 'support'):
                    ...
                    try:
                        ...
                        collection.update(
                            ...
                            metadatas=[{
                                ...
                                'department': self._derive_department(article.category),
                                'source': self._derive_source(article.category),
                    ...
                    try:
                        ...
                        collection.update(
                            ...
                            metadatas=[{
                                ...
                                'department': self._derive_department(faq.category),
                                'source': self._derive_source(faq.category),
    ...
    def _generate_embedding(self, text):
        ...
        try:
            ...
            return [0.0] * 384  # MiniLM produces 384-dim embeddings
